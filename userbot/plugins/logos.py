...
#modifiy by @ViLLAiN_V01 for Heavenbot
#If you kang this plugin please don't forget to give credit to Heavenbot and Hellbot. 
import os
import random
import time
...
async def lg1(heavenevent):
    ...
    text = heavenevent.pattern_match.group(1)
    if len(text) <= 8:
        font_size_ = 150
        strik = 10
    elif len(text) >= 9:
    ...
    await bot.send_file(
        heavenevent.chat_id,
        file_name,
        caption=f"**мα∂ε вү  :** 『😈[{DEFAULTUSER}](tg://user?id={heaven})😈』**\n\n",
    )
    await event.delete()
    try:
        os.remove(file_name)
