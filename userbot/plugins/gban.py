...
async def gban(userbot):
    ...
    await heavenbot.edit(f"`🔥Global Ban Iz Cumin💦.... Wait and watch nigga🚶`")
    ...
    if user:
        ...
        for i in testuserbot:
            try:
                ...
                a += 1
                await heavenbot.edit(f"Gbaning This retard🚶\n\n**Please Wait Few Minutes**😏")
            except:
                b += 1
    else:
        await heavenbot.edit(f"`Either reply to a user or gib me user id/name`")
    try:        
        if gmute(user.id) is False:
            return await heavenbot.edit(f"**Error! User already gbanned.**")
    except:
        pass
    return await heavenbot.edit(
...
async def gunban(userbot):
    ...
    hum = await userbot.client.get_me()
    await heavenbot.edit(f"`Trying to ungban this kiddo...`")
    my_mention = "[{}](tg://user?id={})".format(hum.first_name, hum.id)
    f"@{hum.username}" if hum.username else my_mention
    await userbot.get_chat()
    a = b = 0
    ...
    if user:
        ...
        for i in testuserbot:
            try:
                ...
                a += 1
                await heavenbot.edit(f"Ok! Now Ungbaning this kiddo.\n\n**Please Wait Few Minutes**😏")
            except:
                b += 1
    else:
