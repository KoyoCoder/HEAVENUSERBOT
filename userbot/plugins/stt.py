...
async def _(event):
    ...
    heavenevent = await edit_or_reply(event, "Downloading to my local, for analysis  🙇")
    if event.reply_to_msg_id:
        ...
        if (
            ...
            await heavenevent.edit("Starting analysis, using IBM WatSon Speech To Text")
            headers = {
            ...
            if "results" in r:
                ...
                await heavenevent.edit(string_to_show)
            else:
                await heavenevent.edit(r["error"])
            # now, remove the temporary file
            os.remove(required_file_name)
    else:
        await heavenevent.edit("Reply to a voice message, to get the relevant transcript.")
