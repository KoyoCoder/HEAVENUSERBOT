...
DEFAULTUSERBIO = str(BIO_MSG) if BIO_MSG else "LEGENDARY AF HEAVENBOT"
DEL_TIME_OUT = 60
...
async def _(event):
    if event.fwd_from:
        return
    heaven = await edit_or_reply(event, "`Starting AutoName Please Wait`")
    if event.fwd_from:
        return
    ...
    await heaven.edit(f"Auto Name has been started my Master")
