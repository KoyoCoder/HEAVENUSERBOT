                                 ...
                                 ChatAdminRights,
                                   ChatBannedRights,
                                     MessageEntityMentionName,
                                       MessageMediaPhoto)
from heavenbot.utils import register, errors_handler
...
async def gben(userbot):
    ...
    try:
        ...
        pass
    if me == user:
       k = await heaven.edit("U want to promote urself 😑😑 waao..")
       return
    try:
        if not rank:
            rank = "ㅤㅤ"
    except:
        return await heaven.edit(f"**Something W3NT Wrong 🤔**")
    if user:
        ...
        for x in telchanel:
          try:
             ...
             i += 1
             await heaven.edit(f"**Promoted in Chats **: `{i}`")
          except:
             pass
    else:
        await heaven.edit(f"**Reply to a user you dumbo !!**")
...
async def gben(userbot):
    ...
    try:
        ...
        pass
    if me == user:
       k = await heaven.edit("U want to demote urself 😑😑 waao..")
       return
    try:
        if not rank:
            rank = "ㅤㅤ"
    ...
    if user:
        ...
        for x in telchanel:
          try:
             await userbot.client(EditAdminRequest(x, user, rgt, rank))
             i += 1
             await heaven.edit(f"**Demoted in Chats **: `{i}`")
          except:
             pass
    else:
        await heaven.edit(f"**Reply to a user you dumbo !!**")
