...
from heavenbot import CmdHelp, bot as heavenbot, ALIVE_NAME
from heavenbot.utils import admin_cmd, sudo_cmd, edit_or_reply as eor

DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "Your Lover"

h1m4n5hu0p = heavenbot.uid

heaven = f"[{DEFAULTUSER}](tg://user?id={h1m4n5hu0p})"

@heavenbot.on(admin_cmd(pattern="hvd$"))
@heavenbot.on(sudo_cmd(pattern="hvd$", allow_sudo=True))
...
@heavenbot.on(admin_cmd(pattern="vday$"))
@heavenbot.on(sudo_cmd(pattern="vday$", allow_sudo=True))
async def gif(event):
    if event.fwd_from:
        return
