...
heaven_row = Config.BUTTONS_IN_HELP
heaven_emoji = Config.EMOJI_IN_HELP
...
def button(page, modules):
    ...
    buttons.append(
        [
            custom.Button.inline(
               f"◀️ ᏴᎪᏟᏦ {heaven_emoji}", data=f"page({(max_pages - 1) if page == 0 else (page - 1)})"
            ),
            custom.Button.inline(
               f"•{heaven_emoji} ❌ {heaven_emoji}•", data="close"
            ),
            custom.Button.inline(
               f"{heaven_emoji} ΝᎬХͲ ▶️", data=f"page({0 if page == (max_pages - 1) else page + 1})"
            ),
        ]
    )
...
if Var.TG_BOT_USER_NAME_BF_HER is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        result = None
        query = event.text
        if event.query.user_id == bot.uid and query == "@HeavenBot_Official":
            rev_text = query[::-1]
            veriler = button(0, sorted(CMD_HELP))
            result = await builder.article(
                f"Hey! Only use .help please",
                text=f"**Running HeavenBot**\n\n__Number of plugins installed__ :`{len(CMD_HELP)}`\n**page:** 1/{veriler[0]}",
                buttons=veriler[1],
                link_preview=False,
            )
            ...
            result = builder.article(
                "@HeavenBot_Official",
                text="""**Hey! This is [HeavenBot.](https://t.me/HeavenBot_Official) \nYou can know more about me from the links given below 👇**""",
                buttons=[
                    [
                        custom.Button.url("🔥 CHANNEL 🔥", "https://t.me/HeavenBot_Official"),
                        custom.Button.url(
                            "⚡ GROUP ⚡", "https://t.me/HeavenBot_Support"
                        ),
                    ],
                    [
                        custom.Button.url(
                            "✨ REPO ✨", "https://github.com/VILLAIN-V01/HEAVENBOT"),
                        custom.Button.url
                    (
    ...
    async def page(event):
        ...
        await event.edit(
            f"**Legenday AF** [HeavenBot](https://t.me/HeavenBot_Official) __Working...__\n\n**Number of modules installed :** `{len(CMD_HELP)}`\n**page:** {page + 1}/{veriler[0]}",
            buttons=veriler[1],
            link_preview=False,
        )
