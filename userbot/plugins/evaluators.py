...
async def _(event):
    ...
    heavenevent = await edit_or_reply(event, "`Executing.....`")
...
async def _(event):
    ...
    heavenevent = await edit_or_reply(event, "`Running ...`")
...
CmdHelp("evaluators").add_command(
  ...
  'exec', '<command>', 'Execute a Terminal command on HeavenBot server and shows details'
).add_command(
