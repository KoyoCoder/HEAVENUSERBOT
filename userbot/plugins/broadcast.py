...
async def heavenbroadcast_send(event):
    if event.fwd_from:
        return
    heaveninput_str = event.pattern_match.group(1)
    if not heaveninput_str:
    ...
    heavenevent = await edit_or_reply(
        event,
    ...
    await heavenevent.edit(resultext)
    if BOTLOG:
...
async def heavenbroadcast_add(event):
    if event.fwd_from:
        return
    heaveninput_str = event.pattern_match.group(1)
    if not heaveninput_str:
...
async def heavenbroadcast_remove(event):
    if event.fwd_from:
        return
    heaveninput_str = event.pattern_match.group(1)
    if not heaveninput_str:
...
async def heavenbroadcast_list(event):
    if event.fwd_from:
        return
    heaveninput_str = event.pattern_match.group(1)
    if not heaveninput_str:
    ...
    await edit_or_reply(heavenevent, finaloutput)
...
async def heavenbroadcast_delete(event):
    if event.fwd_from:
        return
    heaveninput_str = event.pattern_match.group(1)
    check1 = sql.num_broadcastlist_chat(heaveninput_str)
    if check1 < 1:
        return await edit_delete(
            event,
            f"Are you sure that there is category {heaveninput_str}",
            parse_mode=parse_pre,
        )
    try:
        sql.del_keyword_broadcastlist(heaveninput_str)
        await edit_or_reply(
            event,
            f"Successfully deleted the category {heaveninput_str}",
            parse_mode=parse_pre,
        )
    except Exception as e:
        await edit_delete(
