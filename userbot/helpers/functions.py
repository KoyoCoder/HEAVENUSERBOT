...
async def admin_groups(heaven):
    heavengroups = []
    async for dialog in heaven.client.iter_dialogs():
...
async def make_gif(event, file):
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await event.client.send_read_acknowledge(conv.chat_id)
            heavenfile = await event.client.download_media(heavenresponse, "./temp")
            return await unzip(heavenfile)
...
async def waifutxt(text, chat_id, reply_to_id, bot, borg):
    ...
    heaven = await sticcers[0].click("me", hide_via=True)
    if heaven:
        await bot.send_file(int(chat_id), heaven, reply_to=reply_to_id)
        await heaven.delete()
